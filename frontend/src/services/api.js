import axios from 'axios';

export const apiBaseUrl = import.meta.env.VITE_API_URL || 'http://localhost:5000/api';

if (import.meta.env.PROD && !import.meta.env.VITE_API_URL) {
  console.warn('VITE_API_URL is not set. The app will try localhost and API calls will fail in production.');
}

export const api = axios.create({
  baseURL: apiBaseUrl
});

const publicApi = axios.create({
  baseURL: apiBaseUrl
});

api.interceptors.request.use((config) => {
  const token = localStorage.getItem('rms_token');
  if (token) config.headers.Authorization = `Bearer ${token}`;
  return config;
});

api.interceptors.response.use(
  (response) => response,
  (error) => {
    if (error.response?.status === 401) {
      localStorage.removeItem('rms_token');
      localStorage.removeItem('rms_user');
      window.dispatchEvent(new Event('rms:auth-expired'));
    }
    return Promise.reject(error);
  }
);

export const endpoints = {
  login: (data) => api.post('/auth/login', data),
  me: () => api.get('/auth/me'),
  dashboard: () => api.get('/dashboard/stats'),
  publicSettings: () => publicApi.get('/public/settings'),
  settings: () => api.get('/settings'),
  updateSettings: (data) => api.put('/settings', data),
  publicMenu: () => publicApi.get('/public/menu'),
  publicMealReviews: (id) => publicApi.get(`/public/menu/${id}/reviews`),
  createPublicMealReview: (id, data) => publicApi.post(`/public/menu/${id}/reviews`, data),
  publicPromotions: () => publicApi.get('/public/promotions'),
  publicMarketing: () => publicApi.get('/public/marketing'),
  publicFlashSale: () => publicApi.get('/public/flash-sale'),
  publicCustomerSession: (data) => publicApi.post('/public/customers/session', data),
  updatePublicCustomer: (id, data) => publicApi.put(`/public/customers/${id}`, data),
  publicCustomerOrders: (id) => publicApi.get(`/public/customers/${id}/orders`),
  submitPromotionRequest: (data) => publicApi.post('/public/promotions', data),
  uploadPublicPromotionImage: (data, config = {}) => publicApi.post('/public/upload/promotion-image', data, { ...config, headers: { 'Content-Type': 'multipart/form-data', ...(config.headers || {}) } }),
  uploadPublicCustomerAvatar: (data, config = {}) => publicApi.post('/public/upload/customer-avatar', data, { ...config, headers: { 'Content-Type': 'multipart/form-data', ...(config.headers || {}) } }),
  createOnlineOrder: (data) => publicApi.post('/public/orders', data),
  publicOnlineOrder: (id) => publicApi.get(`/public/orders/${id}`),
  createReservation: (data) => publicApi.post('/public/reservations', data),
  menuItems: (params) => api.get('/menu-items', { params }),
  createMenuItem: (data) => api.post('/menu-items', data),
  updateMenuItem: (id, data) => api.put(`/menu-items/${id}`, data),
  deleteMenuItem: (id) => api.delete(`/menu-items/${id}`),
  menuCategories: () => api.get('/menu-categories'),
  updateMenuCategory: (id, data) => api.put(`/menu-categories/${id}`, data),
  ingredients: () => api.get('/ingredients'),
  stockItems: (params) => api.get('/stock-items', { params }),
  createStockItem: (data) => api.post('/stock-items', data),
  updateStockItem: (id, data) => api.put(`/stock-items/${id}`, data),
  deleteStockItem: (id) => api.delete(`/stock-items/${id}`),
  stockMovement: (data) => api.post('/stock-movements', data),
  stockMovements: () => api.get('/stock-movements'),
  sales: (params) => api.get('/sales', { params }),
  onlineOrders: () => api.get('/online-orders'),
  updateOnlineOrderStatus: (id, data) => api.put(`/online-orders/${id}/status`, data),
  updateOnlineOrderTracking: (id, data) => api.put(`/online-orders/${id}/tracking`, data),
  deliveryAgents: (params) => api.get('/delivery-agents', { params }),
  createDeliveryAgent: (data) => api.post('/delivery-agents', data),
  assignDeliveryAgent: (id, data) => api.put(`/online-orders/${id}/driver`, data),
  reservations: () => api.get('/reservations'),
  updateReservationStatus: (id, data) => api.put(`/reservations/${id}/status`, data),
  createSale: (data) => api.post('/sales', data),
  debtors: (params) => api.get('/debtors', { params }),
  createDebtor: (data) => api.post('/debtors', data),
  updateDebtor: (id, data) => api.put(`/debtors/${id}`, data),
  deleteDebtor: (id) => api.delete(`/debtors/${id}`),
  expenses: (params) => api.get('/expenses', { params }),
  createExpense: (data) => api.post('/expenses', data),
  updateExpense: (id, data) => api.put(`/expenses/${id}`, data),
  deleteExpense: (id) => api.delete(`/expenses/${id}`),
  expenseCategories: () => api.get('/expense-categories'),
  createExpenseCategory: (data) => api.post('/expense-categories', data),
  uploadReceipt: (data, config = {}) => api.post('/upload/receipt', data, { ...config, headers: { 'Content-Type': 'multipart/form-data', ...(config.headers || {}) } }),
  uploadImage: (data, config = {}) => api.post('/upload/image', data, { ...config, headers: { 'Content-Type': 'multipart/form-data', ...(config.headers || {}) } }),
  salesReport: (params) => api.get('/reports/sales', { params }),
  expensesReport: (params) => api.get('/reports/expenses', { params }),
  businessIntelligence: (params) => api.get('/analytics/business-intelligence', { params }),
  exportBusinessIntelligence: (params) => api.get('/reports/business-intelligence/export', { params, responseType: 'blob' }),
  promotions: (params) => api.get('/promotions', { params }),
  marketingDashboard: () => api.get('/marketing/dashboard'),
  marketingItems: (params) => api.get('/marketing/items', { params }),
  createMarketingItem: (data) => api.post('/marketing/items', data),
  updateMarketingItem: (id, data) => api.put(`/marketing/items/${id}`, data),
  deleteMarketingItem: (id) => api.delete(`/marketing/items/${id}`),
  exportMarketingReport: () => api.get('/marketing/reports/export', { responseType: 'blob' }),
  createPromotion: (data) => api.post('/promotions', data),
  updatePromotion: (id, data) => api.put(`/promotions/${id}`, data),
  deletePromotion: (id) => api.delete(`/promotions/${id}`),
  flashSaleCodes: (params) => api.get('/flash-sale-codes', { params }),
  createFlashSaleCode: (data) => api.post('/flash-sale-codes', data),
  updateFlashSaleCode: (id, data) => api.put(`/flash-sale-codes/${id}`, data),
  deleteFlashSaleCode: (id) => api.delete(`/flash-sale-codes/${id}`),
  users: (params) => api.get('/users', { params }),
  customers: (params) => api.get('/customers', { params }),
  createUser: (data) => api.post('/users', data),
  updateUser: (id, data) => api.put(`/users/${id}`, data),
  deleteUser: (id) => api.delete(`/users/${id}`),
  roles: () => api.get('/roles'),
  suppliers: () => api.get('/suppliers')
};
